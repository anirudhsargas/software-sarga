...
(async () => {
  try {
    ...
    console.log('Resetting passwords for Other Staff members:\n');

    for (const member of staff) {
      // Hash the user_id as password
      const hashedPassword = await bcrypt.hash(member.user_id, 10);
      
      // Update password and set is_first_login to 0
      await conn.query(
        'UPDATE sarga_staff SET password = ?, is_first_login = 0 WHERE id = ?',
        [hashedPassword, member.id]
      );
      
      console.log(`✅ ${member.name} (ID: ${member.id})`);
      console.log(`   User ID: ${member.user_id}`);
      console.log(`   Password: ${member.user_id}`);
      console.log('');
    }
    ...
    staff.forEach(member => {
      console.log(`- ${member.name}: ${member.user_id} / ${member.user_id}`);
    });
