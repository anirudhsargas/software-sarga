...
module.exports = async (connection) => {
  ...
  // 1. Add is_deleted to sarga_inventory
  ...
  if (invCol.length === 0) {
    await connection.query(`ALTER TABLE sarga_inventory ADD COLUMN is_deleted TINYINT(1) NOT NULL DEFAULT 0 AFTER updated_at`);
    await connection.query(`ALTER TABLE sarga_inventory ADD INDEX idx_inventory_is_deleted (is_deleted)`);
  }

  // 2. Add is_deleted to sarga_products
  ...
  if (prodCol.length === 0) {
    await connection.query(`ALTER TABLE sarga_products ADD COLUMN is_deleted TINYINT(1) NOT NULL DEFAULT 0 AFTER is_active`);
    await connection.query(`ALTER TABLE sarga_products ADD INDEX idx_products_is_deleted (is_deleted)`);
  }

  // 3. Add sync_enabled to sarga_products (default TRUE — linked mode)
  ...
  if (syncCol.length === 0) {
    await connection.query(`ALTER TABLE sarga_products ADD COLUMN sync_enabled TINYINT(1) NOT NULL DEFAULT 1 AFTER is_deleted`);
  }

  // 4. Add updated_at to sarga_inventory if missing
  ...
  if (updatedAtCol.length === 0) {
    await connection.query(`ALTER TABLE sarga_inventory ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER is_deleted`);
  }
};
