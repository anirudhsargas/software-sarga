...
router.get('/laser-live', auth.authenticate, auth.authorizeRoles('Admin', 'Accountant', 'Front Office'), async (req, res) => {
    try {
        ...
        let machines = [];
        const machineTypeClause = `LOWER(COALESCE(m.book_type, 'Laser')) = 'laser'`;
        if (!['Admin', 'Accountant'].includes(req.user.role)) {
            const [rows] = await pool.query(
...
router.get('/other-live', auth.authenticate, auth.authorizeRoles('Admin', 'Accountant', 'Front Office'), async (req, res) => {
    try {
        ...
        // 1. Get active photocopy/other machines for this branch.
        let machines = [];
        const machineTypeClause = `LOWER(COALESCE(m.book_type, '')) = 'other'`;
        if (!['Admin', 'Accountant'].includes(req.user.role)) {
            const [rows] = await pool.query(
                `SELECT m.id, m.machine_name, m.machine_type, m.counter_type, m.location, m.branch_id
                 FROM sarga_machines m
        ...
        if (machineIds.length > 0) {
            ...
            if (reportIds.length > 0) {
                ...
                // Group auto-synced entries by payment_id, keep manual entries as-is
                const billGroups = {};
                const manualEntries = [];
        ...
        // 4. Billing payments linked to Other (from customer_payments)
        ...
        const billingEntries = [];
        otherPayments.filter(cp => !coveredPaymentIds.has(Number(cp.id))).forEach(cp => {
        ...
        // 5. Internal transfers affecting Other book
        const [transfers] = await pool.query(
            `SELECT id, amount, from_book_type, to_book_type, note, created_at FROM sarga_internal_transfers WHERE DATE(created_at) = ? AND branch_id = ?`,
            [date, branchId]
        );
        ...
        console.error('Error fetching other live data:', err);
