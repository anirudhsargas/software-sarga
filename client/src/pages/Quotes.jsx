...
const btnStyle = (bg = '#4f46e5') => ({ background: bg, color: bg === 'var(--primary)' ? 'var(--on-accent)' : '#fff', border: 'none', borderRadius: 10, padding: '10px 18px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 14, minHeight: 42, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' });
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                ...
                <div style={{ textAlign: 'right', minWidth: 180, flex: '0 0 auto' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                        {q.status !== 'converted' && (
                            <>
                                <button onClick={() => onEdit(q.id)} className="touch-target" style={btnStyle('#374151')} title="Edit" aria-label={`Edit quote ${q.quote_number}`}><Edit2 size={14} /></button>
                                <button onClick={() => onSend(q)} className="touch-target" style={btnStyle('#3b82f6')} title="Send" aria-label={`Send quote ${q.quote_number}`}><Send size={14} /></button>
                                <button onClick={() => onConvert(q.id)} className="touch-target" style={btnStyle('#22c55e')} title="Convert to Invoice" aria-label={`Convert quote ${q.quote_number} to invoice`}><ArrowRight size={14} /> Invoice</button>
                            </>
                        )}
                        <button onClick={() => onDelete(q.id)} className="touch-target" style={btnStyle('#ef4444')} title="Delete" aria-label={`Delete quote ${q.quote_number}`}><Trash2 size={14} /></button>
                    </div>
                </div>
            </div>
        </div>
