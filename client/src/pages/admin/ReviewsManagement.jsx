    ...
    <div style={{ display: 'inline-flex', gap: 2 }}>
      {STARS.map(s => (
        <button key={s} type="button" onClick={() => interactive && onChange?.(s)}
          style={{ background: 'none', border: 'none', cursor: interactive ? 'pointer' : 'default', padding: 0, color: s <= Math.round(rating) ? '#f59e0b' : '#ddd' }}>
          <StarIcon size={interactive ? 22 : 14} fill={s <= Math.round(rating) ? '#f59e0b' : 'none'} />
        </button>
      ))}
    </div>
  );
            ...
            {filtered.map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '0.75rem 0.5rem', textAlign: 'center' }}>
                  <button className="btn btn-sm" style={{ background: r.is_featured ? '#f59e0b' : 'transparent', color: r.is_featured ? '#fff' : 'var(--text-muted)', border: '1px solid', borderColor: r.is_featured ? '#f59e0b' : 'var(--border)' }} onClick={() => handleToggleFeature(r.id)}>
                    <Star size={12} />
                  </button>
                </td>
                <td style={{ padding: '0.75rem 0.5rem', textAlign: 'center' }}>
