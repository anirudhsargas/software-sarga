...
const STATUSES = [
  ...
  { value: 'under_review', label: 'Under Review', color: 'var(--warning)' },
  { value: 'proof_sent', label: 'Proof Sent', color: 'var(--muted-foreground)' },
  { value: 'approved', label: 'Approved', color: 'var(--muted-foreground)' },
  { value: 'printing', label: 'Printing', color: 'var(--muted-foreground)' },
  { value: 'completed', label: 'Completed', color: 'var(--muted-foreground)' },
  { value: 'cancelled', label: 'Cancelled', color: 'var(--destructive)' },
];
        ...
        {selected && detail && (
          <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '1.25rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  {JSON.parse(detail.files).map((f, i) => (
                    <div key={i} style={{
                      ...
                      <span style={{
                        width: 6, height: 6, borderRadius: '50%', background: f.error ? 'var(--destructive)' : 'var(--success)', flexShrink: 0
                      }} />
            ...
            <button className="btn btn-sm btn-ghost" onClick={() => handleDelete(detail.id)}
              style={{ color: 'var(--destructive)', fontSize: '0.8rem' }}>
              Delete Artwork
            </button>
          </div>
        )}
