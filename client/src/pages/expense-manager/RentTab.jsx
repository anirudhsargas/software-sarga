            ...
            <form onSubmit={submitRent}>
              <div className="em-modal__body">
                <div className="em-form-grid">
                  <div className="em-form-group"><label>Property Name</label><input name="property_name" className="em-input" value={form.property_name} onChange={e => setForm(p => ({ ...p, property_name: e.target.value }))} required /></div>
                  <div className="em-form-group"><label>Location</label><input name="location" className="em-input" value={form.location} onChange={e => setForm(p => ({ ...p, location: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Owner Name</label><input name="owner_name" className="em-input" value={form.owner_name} onChange={e => setForm(p => ({ ...p, owner_name: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Owner Mobile</label><input name="owner_mobile" className="em-input" value={form.owner_mobile} onChange={e => setForm(p => ({ ...p, owner_mobile: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Monthly Rent (₹)</label><input name="monthly_rent" className="em-input" type="number" min="0" value={form.monthly_rent} onChange={e => setForm(p => ({ ...p, monthly_rent: e.target.value }))} required /></div>
                  <div className="em-form-group"><label>Due Day</label><input name="due_day" className="em-input" type="number" min="1" max="31" value={form.due_day} onChange={e => setForm(p => ({ ...p, due_day: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Advance Deposit (₹)</label><input name="advance_deposit" className="em-input" type="number" min="0" value={form.advance_deposit} onChange={e => setForm(p => ({ ...p, advance_deposit: e.target.value }))} /></div>
                  {branches.length > 0 && <div className="em-form-group"><label>Branch</label><select name="branch_id" aria-label="Select option"  className="em-input" value={form.branch_id} onChange={e => setForm(p => ({ ...p, branch_id: e.target.value }))}><option value="">Select Branch</option>{branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>}
                </div>
              </div>
          ...
          <div role="button" tabIndex={0}  className="em-modal" onClick={e => e.stopPropagation()} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); } }}>
            ...
            <form onSubmit={submitRentRequest}>
              <div className="em-modal__body">
                ...
                <div className="em-form-grid">
                  <div className="em-form-group"><label>Property Name *</label><input name="request_property_name" className="em-input" value={requestForm.property_name} onChange={e => setRequestForm(p => ({ ...p, property_name: e.target.value }))} required /></div>
                  <div className="em-form-group"><label>Location</label><input name="request_location" className="em-input" value={requestForm.location} onChange={e => setRequestForm(p => ({ ...p, location: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Owner Name</label><input name="request_owner_name" className="em-input" value={requestForm.owner_name} onChange={e => setRequestForm(p => ({ ...p, owner_name: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Owner Mobile</label><input name="request_owner_mobile" className="em-input" value={requestForm.owner_mobile} onChange={e => setRequestForm(p => ({ ...p, owner_mobile: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Monthly Rent (₹)</label><input name="request_monthly_rent" className="em-input" type="number" min="0" value={requestForm.monthly_rent} onChange={e => setRequestForm(p => ({ ...p, monthly_rent: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Due Day</label><input name="request_due_day" className="em-input" type="number" min="1" max="31" value={requestForm.due_day} onChange={e => setRequestForm(p => ({ ...p, due_day: e.target.value }))} /></div>
                  <div className="em-form-group"><label>Advance Deposit (₹)</label><input name="request_advance_deposit" className="em-input" type="number" min="0" value={requestForm.advance_deposit} onChange={e => setRequestForm(p => ({ ...p, advance_deposit: e.target.value }))} /></div>
                  {branches.length > 0 && <div className="em-form-group"><label>Branch</label><select name="request_branch_id" aria-label="Select option"  className="em-input" value={requestForm.branch_id} onChange={e => setRequestForm(p => ({ ...p, branch_id: e.target.value }))}><option value="">Select Branch</option>{branches.map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>}
                  <div className="em-form-group em-form-group--full"><label>Reason / Notes</label><input name="request_reason" className="em-input" value={requestReason} onChange={e => setRequestReason(e.target.value)} placeholder="Why is this location needed?" /></div>
                </div>
              </div>
