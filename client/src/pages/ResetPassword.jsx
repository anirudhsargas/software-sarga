...
export default function ResetPassword() {
        ...
        return (
            <div style={containerStyle}>
                <div style={{ ...cardStyle, textAlign: 'center' }}>
                    <AlertCircle size={48} style={{ color: 'var(--destructive)', marginBottom: 16 }} />
                    <h2 style={{ margin: '0 0 8px', color: 'var(--foreground)' }}>Invalid or Expired Link</h2>
                    <p style={{ color: 'var(--muted-foreground)' }}>This password reset link is invalid or has expired. Please request a new one.</p>
                    <Link to="/forgot-password" style={{ color: 'var(--accent)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 16 }}>
                        Request New Link
                    </Link>
                </div>
            </div>
        ...
        return (
            <div style={containerStyle}>
                <div style={{ ...cardStyle, textAlign: 'center' }}>
                    ...
                    <p style={{ color: 'var(--muted-foreground)' }}>Your password has been successfully reset. You can now login with your new password.</p>
    ...
    return (
        <div style={containerStyle}>
            <div style={cardStyle}>
                <h2 style={{ margin: '0 0 8px', color: 'var(--foreground)', textAlign: 'center' }}>Reset Password</h2>
                <p style={{ color: 'var(--muted-foreground)', textAlign: 'center', marginBottom: 24 }}>Enter your new password below.</p>
                <form onSubmit={handleSubmit}>
                    <div style={{ marginBottom: 16 }}>
                        <label style={{ fontSize: 14, fontWeight: 600, display: 'block', marginBottom: 6, color: 'var(--muted-foreground)' }}>New Password</label>
                        <div style={{ position: 'relative' }}>
                            <Lock size={18} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--muted-foreground)' }} />
                            <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                                placeholder="Minimum 6 characters" style={{ ...inputStyle, paddingLeft: 40, paddingRight: 40 }} />
                            <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--muted-foreground)' }}>
                                {showPw ? <EyeOff size={18} /> : <Eye size={18} />}
                            </button>
                        </div>
                    </div>
                    <div style={{ marginBottom: 16 }}>
                        <label style={{ fontSize: 14, fontWeight: 600, display: 'block', marginBottom: 6, color: 'var(--muted-foreground)' }}>Confirm Password</label>
                        <div style={{ position: 'relative' }}>
                            <Lock size={18} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--muted-foreground)' }} />
